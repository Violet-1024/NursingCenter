<template>
  <div>
    ...
    <el-header class="bg-lightgray" height="60px">
      <PageTitle :title="'床位管理'" :SecTitle="'床位选择'"></PageTitle>
    </el-header>
    <!--    搜索表单-->
    <div style="margin-top: 20px">
      <el-form :inline="true"  class="demo-form-inline">
        ...
        <el-form-item label="床位等级">
          <el-select  style="width: 240px" v-model="params.bedLevel" placeholder="请选择">
            <el-option
    ...
    <el-table :data="tableData" stripe>
      ...
      <el-table-column label="操作" width="200px">
        <template v-slot="scope">
          <el-button  type="primary" size="small" @click="select(scope.row)">
            <i class="el-icon-edit"></i>选择</el-button>
    ...
    <!--    选择对话框-->
    <el-dialog title="选择床位" custom-class="view-box"
      ...
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="床位等级">
          <el-input v-model="form.bedLevel" placeholder="请输入护理员编号"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    select(row){
      ...
      request.get('/customer/'+id).then(res =>{
        if(res.code==='200'){
          ...
          this.form.custName = res.data.custName;
        }else{
    ...
    update(){
      request.post('bedrecord/save',this.form).then(res => {
        if(res.code === '200'){
